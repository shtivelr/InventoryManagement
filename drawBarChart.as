...
import flash.events.MouseEvent;
	...
	public class drawBarChart extends MovieClip{
		public var TextArray:Array = new Array();
		
		public function drawBarChart(recX:Number, recY:Number, recWidth:Number, recHeight:Number, recScaleY:int){
...
chartBackground.graphics.drawRect(recX, recY, recWidth, recHeight); // (x spacing, y spacing, width, height)
...
recGridLines.graphics.moveTo(recX + recWidth/7, recY + recHeight/7);
recGridLines.graphics.lineTo(recX + recWidth/7, recY + recHeight*6/7);
recGridLines.graphics.lineTo(recX + recWidth*6/7, recY + recHeight*6/7);
...
TextArray[i].x = recX + recWidth*(i+ 1.5)/7 - 5;
...
myTextField.text = "Inventory Level"; 
...
txt4.x = recX + recWidth/7 + labelXAdjust;
...
var i:int;
for (i=6; i > 0; i--){
recGridLines.graphics.moveTo(recX + recWidth*i/7, recY + recHeight*5.9/7); // horizontal tick lines
recGridLines.graphics.lineTo(recX + recWidth*i/7, recY + recHeight*6.1/7);

recGridLines.graphics.moveTo(recX + recWidth*.9/7, recY + recHeight*i/7);  // vertical tick lines
recGridLines.graphics.lineTo(recX + recWidth*1.1/7, recY + recHeight*i/7);
	 }
